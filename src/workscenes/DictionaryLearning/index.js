...
const DictionaryLearning = ({
  ...
  const wordToLearn = async wordId => {
    ...
    if (wordIndex >= 0 && listIndex >= 0) {
      const item = await setLearningLogs(wordsNew[wordIndex]);
      ...
      newWordCount = { newWords: wordsNewList.length, learned: wordCount.learned, toLearn: wordsLearnList.length }
  ...
  const wordToLearnReturn = async wordId => {
    ...
    if (wordIndex >= 0 && listIndex >= 0) {
      const item = await setLearningLogs(wordsKnow[wordIndex]);
      wordsLearn.push(item);
      wordsLearn.sort((itemA, itemB) => {
        let stateA = 0;
        let stateB = 0;
      ...
      newWordCount = { newWords: wordCount.newWords, learned: wordsKnowList.length, toLearn: wordsLearnList.length }
      wordStatRoutine({ words: newWordCount });
    }
  }
  ...
  const wordToNew = async wordId => {
    ...
    if (wordIndex >= 0 && listIndex >= 0) {
      emptyLogs(user.id, wordsLearn[wordIndex].id);
      wordsNew.push(wordsLearn[wordIndex]);
      wordsNew.sort((itemA, itemB) => (itemB.WordEnFrq - itemA.WordEnFrq));
      wordsNewList.push(wordsLearn[wordIndex]);
      wordsNewList.sort((itemA, itemB) => (itemB.WordEnFrq - itemA.WordEnFrq));
      wordsNewUpdateRoutine({ words: wordsNew, wordsList: wordsNewList });
  ...
  const wordToKnow = async wordId => {
    ...
    const listIndex = wordsLearnList.findIndex((item) => item.id === wordId);
    if (wordIndex >= 0 && listIndex >= 0) {
      const item = await setKnowLogs(wordsLearn[wordIndex]);
      wordsKnow.push(item);
  ...
  const wordToKnowSkip = async wordId => {
    ...
    const listIndex = wordsNewList.findIndex((item) => item.id === wordId);
    if (wordIndex >= 0 && listIndex >= 0) {
      const item = await setKnowLogs(wordsNew[wordIndex]);
      wordsKnow.push(item);
      wordsKnow.sort((itemA, itemB) => itemA.WordEn.localeCompare(itemB.WordEn));
      wordsKnowList.push(item);
      ...
      wordsNewList.splice(listIndex, 1);
      wordsNewUpdateRoutine({ words: newWords, wordsList: wordsNewList });
      newWordCount = { newWords: wordsNewList.length, learned: wordsKnowList.length, toLearn: wordCount.toLearn }
      wordStatRoutine({ words: newWordCount });
    }
  }
