...
import { View, StyleSheet, Text, TouchableOpacity, TouchableHighlight, ImageBackground, ActivityIndicator, Platform, Dimensions } from "react-native";
...
const Learning = ({ words, current, listener, diapazon, wordToNew, wordToKnow, learnRoutine, countRoutine, vocabularyRoutine }) => {
  ...
  const [coins] = useState([
    { coin: <None00 style={{ width: 40, height: 40, alignSelf: 'center' }} />, state: 0 },
    ...
    { coin: <GoldAll style={{ width: 40, height: 40, alignSelf: 'center' }} />, state: 16 }
  ])
          ...
          <View style={styles.resultCoins1}>
            <View style={styles.resultCoins}>
              ...
              <Text style={[styles.resultText2, text[data.index] === true ? { color: 'white' } : {}]}>
                {data.item.wordRus[0].WordRu.length > 12 ? data.item.wordRus[0].WordRu.slice(0,12)+ '...' : data.item.wordRus[0].WordRu}
              </Text>
            </View>
